...
export async function GET() {
  try {
    ...
    const response = await axios.get<CMCResponse>('https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest', {
      params: {
        start: 1,
    ...
    const enhancedResponse = {
      ...
      analytics: {
        ...
        marketActivity: {
          bullishCoins: response.data.data.filter((coin) => coin.quote.USD.percent_change_24h > 0).length,
          bearishCoins: response.data.data.filter((coin) => coin.quote.USD.percent_change_24h < 0).length,
...
function calculateMarketSentiment(coins: CoinData[]): string {
...
function getTopGainers(coins: CoinData[], limit: number): Array<{
  id: number;
  name: string;
  symbol: string;
  percent_change_24h: number;
...
function getTopLosers(coins: CoinData[], limit: number): Array<{
  id: number;
  name: string;
  symbol: string;
  percent_change_24h: number;
